...
unsigned int currentOutputLimit = MAX_BUFFER_SIZE;	// default max size of current output base
...
char* GetCommandArg(char* ptr, char* buffer,FunctionResult& result,unsigned int control) 
{
	...
	unsigned int size = outputsize - (buffer - currentOutputBase); // how much used
	if (size > outputsize) size = MAX_WORD_SIZE * 4; // arbitrary assumption
